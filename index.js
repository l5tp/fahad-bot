...
// بيانات الـ 10 بوتات مرتبة بدقة مع إيدي الرومات وتوكناتهم
const botsConfig = [
...
// تشغيل البوتات بفاصل زمني (ثانيتين) لمنع ضغط الاستضافة وخطأ SIGTERM
...
function startBot(config) {
    ...
    client.once('ready', async () => {
        ...
        try {
            ...
            if (channel && channel.isVoiceBased()) {
                ...
                console.log(`[🎧 BOT #${config.id}] دخل الروم المخصص له بدقة (ID: ${config.channelId})`);
            }
        } catch (err) {
    ...
    client.on('messageCreate', async message => {
        ...
        // 1. شرط أساسي: يجب أن يكون الشخص المتحدث متواجداً في نفس الروم الصوتي الخاص بهذا البوت تماماً
        const memberVoiceChannel = message.member?.voice.channel;
        if (!memberVoiceChannel || memberVoiceChannel.id !== config.channelId) {
            // يتجاهل الرسالة إذا لم يكن العضو في روم هذا البوت المحدد
            return;
        }
        ...
        if (command === 'play') {
            ...
            // التأكد من الاتصال بالروم المخصص
            checkAndConnectVoice(queue, config, message.guild);
        ...
        // أمر رقصني (!رقصني / !dance)
            ...
            try {
                ...
                message.reply(`❌ بوت #${config.id}: صار خطأ بسيط.`);
            }
        }

        // أمر التخطي (!skip)
        else if (command === 'skip') {
            if (!queue.isPlaying) return message.reply(`❌ بوت #${config.id}: مافيه شي شغال حالياً!`);
            queue.player.stop();
...
// دالة مساعدة لضمان اتصال البوت برومه المحدد دائماً
async function checkAndConnectVoice(queue, config, guild) {
...
async function playNextSong(queue, config) {
    ...
    try {
        ...
        if (queue.textChannel) {
            queue.textChannel.send(`🎶 (بوت #${config.id}) جاري الآن تشغيل: **${currentSong.title}**`);
        }
...
// حماية شاملة لمنع انهيار السكربت
