...
function setupLockedBot(id, token, targetChannelId) {
    ...
    client.once('ready', async () => {
        console.log(`[🔒 BOT #${id}] متصل بنجاح: ${client.user.tag}`);
        // محاولة الدخول الفوري مع إعادة المحاولة لضمان النجاح
        await lockAndJoinVoice(client, state, targetChannelId);
        
        // إعادة التأكيد بعد ثانيتين لضمان استقرار الدخول
        setTimeout(() => {
            lockAndJoinVoice(client, state, targetChannelId);
        }, 2000);
    });

    // حماية وقفل صوتي صارم: لو حاول أي شخص نقله، يرجعه لرومه فوراً
    ...
    client.on('messageCreate', async message => {
        ...
        // الشرط الصارم: العضو لازم يكون بنفس روم هذا البوت بالذات
...
// دالة محسنة ومباشرة لجلب الروم والدخول إليه دون أخطاء
...
async function playSong(state, id, channelId, client) {
    ...
    try {
        await lockAndJoinVoice(client, state, channelId);
        if (state.connection) {
...
// =====================================================================
// تشغيل البوتات بفواصل زمنية دقيقة لضمان دخولهم جميعاً بدون ضغط على السيرفر
// =====================================================================
...
// حماية شاملة لمنع السكربت من الانطفاء
