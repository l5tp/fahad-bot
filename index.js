...
// تشغيل البوتات بفاصل زمني (ثانيتين بين كل بوت) لمنع الانهيار وضمان دخول كل بوت رومه المخصص
...
function startBot(config) {
    ...
    if (!token) {
        console.log(`⚠️ تخطي البوت رقم ${config.id}: متغير البيئة ${config.tokenEnv} غير موجود أو لم يتم إضافته.`);
        return;
    }
    ...
    client.once('ready', async () => {
        ...
        try {
            ...
            if (channel && channel.isVoiceBased()) {
                ...
                console.log(`[🎧 BOT #${config.id}] دخل الروم الصوتي المحدد برقم ID: ${config.channelId} بنجاح!`);
            } else {
                console.log(`[❌ BOT #${config.id}] لم يتم العثور على الروم الصوتي المطابق للـ ID المحدد.`);
            }
        } catch (err) {
    ...
    client.on('messageCreate', async message => {
        ...
        if (command === 'play') {
            ...
            // التأكد من الاتصال برومه الصوتي المحدد خصيصاً له
...
// دالة تشغيل الأغاني المستقلة لكل بوت في رومه بدقة
...
// حماية لمنع انهيار السكربت بالكامل
