...
// جلب التوكن والتحقق منه لعدم حدوث أخطاء ترويسة الاتصال
const token = process.env.TOKEN;

if (!token || token.trim() === "") {
    console.error("[ERROR] لم يتم العثور على توكن البوت في متغيرات البيئة TOKEN!");
}

// إعداد لوحة التحكم الويب العامة
const app = express();
...
// تسجيل الدخول مع التأكد من نظافة التوكن
if (token) {
    client.login(token.trim());
} else {
    console.error("[FATAL] لا يمكن تسجيل الدخول لعدم توفر التوكن.");
}
