    ...
    if (cmd === 'play' || cmd === 'رقصني') {
        ...
        try {
            ...
            if (!q.startsWith('http')) {
                ...
                if (!video) return m.edit('❌ لم يتم العثور على نتائج.');
                videoUrl = video.url;
                title = video.title;
...
async function playNext(st, chId, c) {
    ...
    try {
        ...
        // تصميم لوحة التحكم (Embed) مع صورة البنر والمدة واسم الطالب
        ...
        // الأزرار: إيقاف مؤقت، متابعة، تخطي، إيقاف، رفع صوت، خفض صوت
