    ...
    if (cmd === 'play' || cmd === 'رقصني') {
        ...
        try {
            ...
            if (!q.startsWith('http')) {
                ...
                if (!video) { await m.edit('❌ لم يتم العثور على نتائج مطابقة!'); return; }
                videoUrl = video.url;
                title = video.title;
...
async function playNext(st, chId, c) {
    ...
    try {
        ...
        // استخدام خيارات توافقية تمنع خطأ صيغ يوتيوب
        ...
        if (st.textChannel) st.textChannel.send(`❌ تعذر تشغيل الأغنية (${song.title}).`).catch(()=>{});
        st.songs.shift(); 
        playNext(st, chId, c); 
    }
}
