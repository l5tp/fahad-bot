    ...
    if (cmd === 'play' || cmd === 'رقصني') {
        ...
        try {
            ...
            if (!q.startsWith('http')) {
                ...
                if (!video) {
                    await m.edit('❌ لم يتم العثور على نتائج مطابقة!');
                    return;
                }
                videoUrl = video.url;
                title = video.title;
                ...
                const videoInfo = await yts(q);
                title = videoInfo.title || 'رابط يوتيوب';
                thumbnail = videoInfo.thumbnail || '';
                duration = videoInfo.timestamp || 'غير معروف';
            }
            ...
            console.error('خطأ في جلب الأغنية:', e);
            m.edit(`❌ حدث خطأ أثناء جلب الأغنية: \`${e.message || e}\``); 
        }
    } else if (cmd === 'skip') {
...
async function playNext(st, chId, c) {
    ...
    try {
        ...
        if (st.textChannel) {
            if (st.currentMessage) {
                st.currentMessage.edit({ embeds: [embed], components: [row1, row2] }).catch(async () => { 
                    st.currentMessage = a_wait_send(st.textChannel, embed, row1, row2);
                });
            } else {
        ...
        console.error('خطأ أثناء تشغيل الأغنية:', e);
        if (st.textChannel) st.textChannel.send(`❌ تعذر تشغيل الأغنية (${song.title}) بسبب خطأ تقني.`).catch(()=>{});
        st.songs.shift(); 
        playNext(st, chId, c); 
    }
}
