...
client.on('messageCreate', async msg => {
    ...
    if (cmd === 'play' || cmd === 'رقصني') {
        ...
        const searchingMsg = await msg.reply('🔍 جاري البحث...');
        ...
        try {
            ...
            state.songs.push({ 
                ...
                duration: track.timestamp || 'مباشر', 
...
async function playNext(st, chId, c) {
    ...
    try {
        ...
        // محاولة جلب معلومات الفيديو وتجاوز قيود يوتيوب عبر إجبار الـ streaming على استخدام صيغة فيديو مباشرة
        let source;
        try {
            let info = await play.video_info(song.url);
            source = await play.stream_from_info(info, { quality: 0 });
        } catch (err1) {
            try {
                source = await play.stream(song.url, { quality: 'highestaudio' });
            } catch (err2) {
                // إذا رفض يوتيوب نهائياً، نجلب رابط مباشر بديل من يوتيوب عبر محول داخلي
                const ytStream = await play.search(song.url, { limit: 1, source: { youtube: 'video' } });
                source = await play.stream(song.url);
            }
        }
        ...
        if (st.textChannel) st.textChannel.send(`❌ تعذر تشغيل (${song.title}) بسبب حظر يوتيوب للسيرفر.`).catch(()=>{});
        st.songs.shift(); 
        playNext(st, chId, c); 
    }
}
