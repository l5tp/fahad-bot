...
const CHANNEL_ID = '1545466820193161327'; // حط آيدي روم 10 هنا إذا تبي تغيره
...
client.on('messageCreate', async msg => {
    ...
    if (cmd === 'play' || cmd === 'رقصني') {
        ...
        try {
            let videoUrl = '', title = q, thumbnail = '', duration = 'مباشر';
            if (!q.startsWith('http')) {
                const searchResult = await yts(q);
                const video = searchResult.videos && searchResult.videos[0];
                if (!video) { await m.edit('❌ لم يتم العثور على نتائج!'); return; }
                videoUrl = video.url;
                title = video.title;
            ...
            if (!videoUrl) return m.edit('❌ حدث خطأ في جلب الرابط.');
...
async function playNext(st, chId, c) {
    ...
    try {
        ...
        const streamData = await playdl.stream(song.url, { discordPlayerCompatibility: true });
