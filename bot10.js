...
client.on('messageCreate', async msg => {
    ...
    if (cmd === 'play' || cmd === 'رقصني') {
        ...
        const m = await msg.reply('🔍 جاري البحث في ساوند كلاود...');
        try {
            let trackUrl = '', title = q, thumbnail = '', duration = 'مباشر';
            
            if (!q.startsWith('http')) {
                // البحث المباشر في ساوند كلاود
                const searchResults = await playdl.search(q, { limit: 1, source: { soundcloud: 'tracks' } });
                const track = searchResults[0];
                if (!track) { await m.edit('❌ لم يتم العثور على نتائج في ساوند كلاود!'); return; }
                trackUrl = track.url;
                title = track.title;
                thumbnail = track.thumbnail;
                // تحويل المدة من ثواني إلى دقائق إن وجدت
                duration = track.durationRaw || 'مباشر';
            } else {
                trackUrl = q;
                const trackInfo = await playdl.soundcloud(q);
                title = trackInfo.name;
                thumbnail = trackInfo.thumbnail || '';
                duration = trackInfo.durationRaw || 'غير معروف';
            }
            
            if (!trackUrl) return m.edit('❌ حدث خطأ في جلب رابط ساوند كلاود.');
...
async function playNext(st, chId, c) {
    ...
    try {
        ...
        // سحب الصوت مباشرة من ساوند كلاود بدون حظر
        ...
        const embed = new EmbedBuilder()
            .setColor('#FF5500') // لون ساوند كلاود البرتقالي المميز
            .setTitle('🎶 مشغل ساوند كلاود - SoundCloud')
