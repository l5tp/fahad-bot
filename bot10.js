...
const yts = require('yt-search'); // مكتبة بحث قوية ومستقرة جداً
...
client.on('messageCreate', async msg => {
    ...
    if (cmd === 'play' || cmd === 'رقصني') {
        ...
        const searchingMsg = await msg.reply('🔍 جاري البحث عن الطلب...');

        try {
            // استخدام مكتبة yts للبحث بدقة وسرعة عالية جداً
            const searchResult = await yts(query);
            const videos = searchResult.videos;

            if (!videos || videos.length === 0) {
                return searchingMsg.edit('❌ لم يتم العثور على أي نتائج مطابقة لبحثك.');
            }

            const track = videos[0]; // أخذ أول نتيجة وأدقها
            ...
            searchingMsg.edit('❌ حدث خطأ أثناء البحث، حاول مرة أخرى.').catch(()=>{});
        }
    } else if (cmd === 'skip') {
