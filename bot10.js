...
async function playNext(st, chId, c) {
    ...
    try {
        ...
        // استخدام خيارات إضافية لتجاوز حظر الـ IP وسحب الصوت بدقة
        let streamData;
        try {
            streamData = await play.stream(song.url, { quality: 2, discordPlayerCompatibility: true });
        } catch (err) {
            // محاولة بديلة في حال فشل الطريقة الأولى
            streamData = await play.stream(song.url);
        }
